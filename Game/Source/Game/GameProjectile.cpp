...
void AGameProjectile::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
	...
	//	//GEngine->AddOnScreenDebugMessage(-1, 5.f, FColor::Red, FString::Printf(TEXT("%s"), *OtherActor->GetName()));
	//	Destroy();
	//}

	//// Check if the hit actor is of type BP_AllyActor or BP_EnemyActor
	//if (OtherActor)
	//{
	//	AAllyActor* AllyActor = Cast<AAllyActor>(OtherActor);
	//	AEnemyActor* EnemyActor = Cast<AEnemyActor>(OtherActor);

	//	if (AllyActor || EnemyActor)
	//	{
